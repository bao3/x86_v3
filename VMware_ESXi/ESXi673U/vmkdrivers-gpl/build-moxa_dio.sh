...
make_fakeroot()
{
	rm -rf moxa_dio_fakeroot/usr \
	moxa_dio_fakeroot/sig.pkcs7 \
	moxa_dio_fakeroot/DA820DIO
	
	mkdir -p moxa_dio_fakeroot/usr/lib/vmware/vmkmod

	cp -a BLD/build/vmkdriver-moxa_dio-CUR/release/vmkernel64/moxa_dio moxa_dio_fakeroot/usr/lib/vmware/vmkmod/
	chmod +t  moxa_dio_fakeroot/etc/init.d/da820dio

	(cd moxa_dio_fakeroot; \
	tar -czf DA820DIO.tgz usr etc; \
	mv DA820DIO.tgz DA820DIO; \
	touch sig.pkcs7)
}
...
make_vib()
{
	# Collect all the files in moxa_dio_fakeroot/
	make_fakeroot
	...
	# Please copy the moxa_dio_fakeroot to CentOS 6 32-bits and use the vibauthor to create the vib file.
	#(cd moxa_dio_fakeroot; \
	#ar crv ../DA820DIO.vib descriptor.xml sig.pkcs7 DA820DIO)
}
...
#I_SYSTEM=/build/toolchain/lin32/gcc-4.6.3-2/lib/gcc/x86_64-linux5.0/4.6.3/include
...
-DCONFIG_SMP \
-DGPLED_CODE \
-DKBUILD_MODNAME=\"moxa_dio\" \
-DMODULE \
-DLINUX_MODULE_AUX_HEAP_NAME=vmklnx_moxa_dio \
-DLINUX_MODULE_HEAP_INITIAL=1024*100 \
-DLINUX_MODULE_HEAP_MAX=1024*4096 \
-DLINUX_MODULE_HEAP_NAME=vmklnx_moxa_dio \
-DNO_FLOATING_POINT \
...
$GREP -v -e "SED" build-moxa_dio.sh \
| $GREP -o -e "-o [^ ]*\."            \
| $SED -e 's?-o \(.*\)/[^/]*\.?\1?'   \
| $GREP -v -e "\*"                    \
| $XARGS $MKDIR -p
...
$CC $CFLAGS -c -o BLD/build/vmkdriver-moxa_dio-CUR/release/vmkernel64/SUBDIRS/vmkdrivers/src_9/drivers/char/moxa_dio/moxa_dio.o vmkdrivers/src_9/drivers/char/moxa_dio/moxa_dio.c
$CC $CFLAGS -c -o BLD/build/vmkdriver-moxa_dio-CUR/release/vmkernel64/SUBDIRS/vmkdrivers/src_92/common/vmklinux_module.o vmkdrivers/src_92/common/vmklinux_module.c

$LD -r -o BLD/build/vmkdriver-moxa_dio-CUR/release/vmkernel64/moxa_dio \
--whole-archive \
BLD/build/vmkdriver-moxa_dio-CUR/release/vmkernel64/SUBDIRS/vmkdrivers/src_9/drivers/char/moxa_dio/moxa_dio.o \
BLD/build/vmkdriver-moxa_dio-CUR/release/vmkernel64/SUBDIRS/vmkdrivers/src_92/common/vmklinux_module.o
